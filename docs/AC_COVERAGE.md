...
| — | HU-06: lead Kanban board. AC1 card with name, property, last interaction; AC2 move between stages manually or automatically; AC3 filter by property, creation date, status | AC1 `GET /leads` returns `LeadCard` (`client_name`, `listing_address`, `operation_type`, `asking_price`, `last_interaction`) from one query (`services/lead.list_leads`, `LATERAL` on `idx_interaction_lead`); AC2 `POST /leads/{id}/transitions` + the calendar's automatic stage moves (009); AC3 `property_id`, `created_from`/`created_to` (inclusive, `APP_TIMEZONE`), `stage`, `listing_id`, `agent_id`. The board UI itself is not in this repo | `tests/test_leads_board.py`, `tests/test_tenancy.py` (`test_board_filters_never_leak_across_agencies`), `tests/test_transitions.py` |
