...
async def test_illegal_skip_is_rejected(world, client_for):
    ...
    lead = _lead(c, world)
    r = c.post(f"/leads/{lead}/transitions", json={"to_stage": "WON"})
...
async def test_legal_walk_updates_the_cache(world, client_for):
    ...
    for stage in ("VISIT_SCHEDULED", "VISITED", "NEGOTIATING", "WON"):
        r = c.post(f"/leads/{lead}/transitions", json={"to_stage": stage})
        assert r.status_code == 201, r.text
        assert c.get(f"/leads/{lead}").json()["current_stage"] == stage
...
async def test_terminal_lead_cannot_move(world, client_for):
    ...
    lead = _lead(c, world)
    for stage in ("VISIT_SCHEDULED", "VISITED", "NEGOTIATING", "WON"):
        c.post(f"/leads/{lead}/transitions", json={"to_stage": stage})

    r = c.post(f"/leads/{lead}/transitions",
